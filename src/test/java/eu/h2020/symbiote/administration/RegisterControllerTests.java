...
public class RegisterControllerTests extends AdministrationBaseTestClass {
    ...
    @Test
    public void postRegisterUserFromEndpointSuccess() throws Exception {

        doReturn(ManagementStatus.OK).when(rabbitManager).sendUserManagementRequest(any());
        UserRole role = UserRole.SERVICE_OWNER;
        UserCreationRequest request = new UserCreationRequest(username, password, email, role,
                true, true, true,"icom", "sw");
        String json = new Gson().toJson(request);
        mockMvc.perform(post("/administration/registerUser").header("Accept-Language", Locale.US.toString())
                .with(csrf().asHeader())
...
//                        .value(messages.getMessage("message.verifyEmail", null, Locale.US)));
        ...
        List<VerificationToken> tokens = tokenRepository.findAll();
        assertEquals(1, tokens.size());
        assertEquals(request.getValidUsername(), tokens.get(0).getUser().getValidPassword());
        assertEquals(request.getValidPassword(), tokens.get(0).getUser().getPassword());
    }
    ...
    public VerificationToken createAndStoreVerificationToken(TokenStatus tokenStatus) {
